...
_wjson& _wjson::add(std::string_view name, const _picture& b, uint c00, uint cc1)
{
	...
	{
		pak = true;
		auto data = b.scan_line(0);
		for (i64 i = 0; i < 576; i++)
			if ((data[i] != c00) && (data[i] != cc1))
			{
				pak = false;
				break;
			}
	...
	if (!pak)
	{
		for (int j = 0; j < b.size.y; j++) add_mem(b.scan_line(j), b.size.x * 4ULL);
	}
	else
	{
		uchar a[72] = {};
...
void _rjson::read(std::string_view name, _picture& b, uint c00, uint cc1)
{
	...
	for (int j = 0; j < temp.size(); j++)
		if (!string_to_mem(temp[j], b.scan_line(j), b.size.x * 4))
		{
			error = 12;
			return;
		}
...
{
	const char* ss = read_string(name);
	if (error || null) return;
